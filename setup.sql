...
GRANT CREATE STREAMLIT ON SCHEMA CUST_SUPPORT_DEMO.AGENTS TO ROLE EVAL_ROLE;
...
-- Eval data
INSERT INTO EVAL_DATA (INPUT_QUERY, GROUND_TRUTH_DATA)
SELECT $1, PARSE_JSON($2)
FROM @AGENT_OBS_DEMO_GIT_REPO/branches/main/data/expanded_eval_data.csv (FILE_FORMAT=>SUPPORT_AGENT_CSV_FORMAT);
...
CALL SYSTEM$CREATE_EVALUATION_DATASET(
    ...
    'CUST_SUPPORT_DEMO.AGENTS.SUPPORT_AGENT_EVAL_DATASET',
...
CREATE OR REPLACE STAGE CUST_SUPPORT_DEMO.AGENTS.EVAL_CONFIG_STAGE
  ...
  -- Ensure using latest config files from GitHub
ALTER GIT REPOSITORY AGENT_OBS_DEMO_GIT_REPO FETCH;
...
-- 8. Streamlit Observability Dashboard
ALTER GIT REPOSITORY AGENT_OBS_DEMO_GIT_REPO FETCH;

USE ROLE ACCOUNTADMIN;

CREATE OR REPLACE STREAMLIT CUST_SUPPORT_DEMO.AGENTS.AGENT_OBSERVABILITY_DASHBOARD
  FROM '@CUST_SUPPORT_DEMO.AGENTS.AGENT_OBS_DEMO_GIT_REPO/branches/main/'
  MAIN_FILE = 'agent_observability_dashboard.py'
  QUERY_WAREHOUSE = COMPUTE_WH
  TITLE = 'Agent Observability Dashboard'
  COMMENT = 'Compares Cortex Agent vs LangGraph Agent via Snowflake AI Observability';

ALTER STREAMLIT CUST_SUPPORT_DEMO.AGENTS.AGENT_OBSERVABILITY_DASHBOARD ADD LIVE VERSION FROM LAST;

GRANT USAGE ON CORTEX SEARCH SERVICE CASE_SEARCH_SERVICE TO ROLE ACCOUNTADMIN;
GRANT ALL ON SEMANTIC VIEW SUPPORT_ANALYTICS TO ROLE ACCOUNTADMIN;